...
public class AuthController {
    ...
    public ResponseEntity<UserResponseDTO> register(@RequestBody @Valid UserRegisterRequest request){
        CreateUserCommand userCommand = new CreateUserCommand(request.name(), request.email(), request.password());
        User user = useCase.execute(userCommand);
        UserResponseDTO responseDTO = new UserResponseDTO(user.getId().value().toString(),user.getName(), user.getEmail().value(),user.getStatus().name(),user.getCreatedAt(),user.getUpdatedAt());
        return ResponseEntity.created(URI.create("/api/v1/users/"+responseDTO.id())).body(responseDTO);
    }
}
