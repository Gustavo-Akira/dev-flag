...
public class AuthRestControllerAdvice {
  ...
  public ResponseEntity<ProblemDetail> emailAlreadyOnUse(
      EmailAlreadyOnUse ex, HttpServletRequest request) {
    HttpStatus status = HttpStatus.CONFLICT;
    return ResponseEntity.status(status).body(ProblemDetail.of(status, ex.getMessage(), request));
  }
}
